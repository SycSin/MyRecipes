...
export default {
  ...
  methods: {
    async submitForm() {
      const response = await axios.post('https://myrecipes-backend.myrecipes.svc.cluster.local:3000/auth/signup', this.form)
    ...
    async deleteAccount() {
      await axios.delete(`https://myrecipes-backend.myrecipes.svc.cluster.local:3000/users/${this.id}`, config)
          .then(() => localStorage.removeItem('token'))
          .finally(() => this.$root.$emit('logout'))
    },
    updateAccount() {
      axios.put(`https://myrecipes-backend.myrecipes.svc.cluster.local:3000/users/${this.id}`, {
  ...
  created() {
    axios.get(`https://myrecipes-backend.myrecipes.svc.cluster.local:3000/auth/getSelf/${localStorage.getItem('token')}`, config
    ).then(
        (response) => {
